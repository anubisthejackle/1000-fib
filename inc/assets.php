...
define( 'THOUSAND_FIB_ASSET_MAP', read_asset_map( dirname( __DIR__ ) . '/build/assetMap.json' ) );
define( 'THOUSAND_FIB_ASSET_MODE', THOUSAND_FIB_ASSET_MAP['mode'] ?? 'production' );
...
function get_asset_property( string $asset, string $prop ) : ?string {
	...
	$asset_property = THOUSAND_FIB_ASSET_MAP[ $entrypoint ][ $type ][ $prop ] ?? null;
...
function inline_locale_data( string $to_handle ) {
	...
	wp_add_inline_script(
		$to_handle,
		'wp.i18n.setLocaleData( ' . wp_json_encode( $locale_data ) . ", 'thousand-fib' );"
	);
}
